...
Tested on 200-step synthetic price series with fixed random seeds. Full methodology in [EVALUATION.md](EVALUATION.md).
...
Rather than hardcoding market names, `discover_markets()` in `market_pair.py` scans the full live Polymarket catalogue on each refresh. `classify_market()` in `layer0_classifier.py` tags each contract as `crypto_level`, `rate_decision`, `equity_level`, or `unsupported` using regex patterns with word-boundary guards. Contracts pass three filters before a `MarketPair` is instantiated: minimum volume ($50k), minimum days to expiry (30), and a Black-Scholes uncertainty window (professional probability between 2% and 98%). Within each `(type, underlying)` group the highest-volume contract wins; the scanner table is populated entirely from what discovery returns. Adding support for a new asset requires only extending the classifier patterns — no new loader functions or dashboard code. The roadmap for extending auto-discovery to equity index contracts and additional assets is in [EXPANSION_ROADMAP.md](EXPANSION_ROADMAP.md).
...
All backtests use fixed random seeds for reproducibility (`layer2_avellaneda.py`: `seed=42, p0=0.72`; `layer3_calibration.py`: `seed=7, p_true=0.30`). See [EVALUATION.md](EVALUATION.md) for the full results tables and methodology.
