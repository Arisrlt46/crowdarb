...
- **Crypto:** Black-Scholes N(d₂) on the asset's price level, using Deribit implied volatility (90-day realized fallback).
...
- **`interpret(sig) → Interpretation`** — pure, deterministic, no API calls. Classifies the crowd-vs-professional gap against three thresholds (3 / 8 / 15 pp → aligned / modest / notable / large), sets direction, and builds one sentence per fact present in the signals bundle: source probabilities, weighted blend, reservation-price tilt vs mid, spread, and any LLM likelihood ratio.
- **`compact_read(sig) → str`** — one-line scanner label, e.g. `"Crowd 21 pts high"`.
- **`narrate(sig, interp, client) → str`** — optional LLM paragraph (2–3 sentences, plain English, no jargon) for explicit user requests. One API call per click; cached in session state.

`GLOSSARY` exports 10 jargon-free definitions wired into `st.metric(help=...)` tooltips and a dashboard expander.

## Scanner — real-time auto-discovery

`discover_markets()` scans ~2,100 live Polymarket markets, classifies each (crypto level / rate decision / equity level / unsupported), parses strike and date, filters by volume, expiry, and a Black-Scholes sanity band, builds a MarketPair for each survivor, and ranks them by crowd-vs-professional gap. The dashboard renders the ranked table (with a `"Read"` column from Layer 5), metric tooltips, and a per-market detail panel with a full Layer 5 interpretation.
...
`run_crowdarb(market)` runs Layers 1–5 on any MarketPair with no market-specific logic in the pipeline body. Adding a market is one class of roughly 20 lines; the entire engine then works on it automatically.
...
- A monthly API spend cap and disabled auto-reload bound the cost of the public LLM feature. The Layer 5 narrative call is opt-in and cached per market per session.
